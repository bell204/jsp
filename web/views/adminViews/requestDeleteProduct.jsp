...
<% if(loginUser!= null && loginUser.getC_id().equals("admin")){ %>
	<table class="table1">
  		...
  		<tr class="tal">
  			<td id="tal0"><h5 id="talId"><%= prList.get(i).getP_name() %></h5></td>
  			<td id="tal1"><h5><%= prList.get(i).getC_code() %></h5></td>
  			<td id="tal2"><h5><%= prList.get(i).getP_type() %></h5></td>
  			<td id="tal3"><h5><%= prList.get(i).getDealingCount() %></h5></td>
  	...
  	<script>
  		$(".tal").click(function (){
  			...
  			console.log(p_code);
  			location.href="/p/selldetail?p_code="+p_code; //상품 상세페이지 (버튼 만들고 admin_yn = 'y'로 만들어주기)
  		});
  	</script>
<%}else{ 
