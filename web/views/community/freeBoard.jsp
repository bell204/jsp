	...
	<div id='A_Container_Wrap'>
		<div id='A_Container'>
			...
			<div id="A_Container_C">
				...
				<ul class="pager">
					<li><a
						href="<%=request.getContextPath()%>/selectList.bo?currentPage=1">First</a></li>
					<%if (currentPage <= 1) {%>
					<li><a href="#">Previous</a></li>
					<%} else {%>
					<li><a
						href="<%=request.getContextPath()%>/selectList.bo?currentPage=<%=currentPage - 1%>">Previous</a></li>
					<%
						}
					%>
					<%
					...
					%>
					<li><a href="<%=request.getContextPath()%>/selectList.bo?currentPage=<%=p%>" class="active"><%=p%></a></li>
					<% }%>
					<%}%>
					<% if (currentPage >= maxPage) {%>
					<li><a href="#">Next</a></li>
					<%} else {%>
					<li><a href="<%=request.getContextPath()%>/selectList.bo?currentPage=<%=currentPage + 1%>">Next</a></li>
					<%}%>
					<li><a href="<%=request.getContextPath()%>/selectList.bo?currentPage=<%=maxPage%>">Last</a></li>
					
					<!-- 작성 버튼 -->
					<%if (loginUser != null ) {%>
					<li style="float: right;"><button
							onclick="location.href='/p/views/community/freeBoardInsert.jsp'"
