...
<!-- 
   ...
   <%}else{ %>
   <li><a href="<%= request.getContextPath() %>/selectList.no?currentPage=<%= currentPage-1 %>">Previous</a></li>
   <%} %>
  ...
  <li><a href="#">Next</a></li>
 -->
 <li><a href="<%=request.getContextPath() %>/selectList.no?currentPage=1">First</a></li>
 <% if(currentPage<=1){ %>
   <li><a href="#">Previous</a></li>
   <%}else{ %>
   <li><a href="<%= request.getContextPath() %>/selectList.no?currentPage=<%= currentPage-1 %>">Previous</a></li>
   ...
   <% for(int p=startPage; p<=endPage; p++){
      ...
      %>
           <li><a href="#" class="active"><%= p %></a></li>
   <%}else{ %>
           <li><a href="<%=request.getContextPath() %>/selectList.no?currentPage=<%= p %>" class="active"><%= p %></a></li>
   <%} %>
   <%} %>
  ...
  <li><a href="#">Next</a></li>
	<%}else{ %>
  <li><a href="<%= request.getContextPath()%>/selectList.no?currentPage=<%= currentPage+1%>">Next</a></li>
	<%} %> 
	<li><a href="<%= request.getContextPath()%>/selectList.no?currentPage=<%= maxPage %>">Last</a></li>
	
	
	<%-- 공시사항 등록창 --%>  
  <% if(loginUser!=null && loginUser.getC_id().equals("admin")){ %>
  <li style="float:right;"><button onclick="location.href='/p/views/community/noticeInsert.jsp'" style="background:skyblue; border-radius:5px; color:white; border:none; width:100px; height:30px;">작성하기</button></li>
  <%} %>
  
   </ul>
  <input type="hidden" value="<%= loginUser.getC_code() %>" id="userCode">
