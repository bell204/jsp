 ...
 <% for(product p : list) {%>
        ...
        <!-- if for 문 들어갈 곳 -->
           ...
           <!--/사진테두리-->
            ...
            <p style="color: #6393b4;" id='cate1'>
           <%= p.getCate1()%>><%= p.getCate2()%>><%=p.getCate3()%>><%=p.getCate4()%></p>
            <h3 class="font_subject2"><a href="<%=request.getContextPath()%>/detailCycle?p_code=<%=p.getP_code() %>&cate=<%= p.getCate1()%>><%= p.getCate2()%>><%=p.getCate3()%>><%=p.getCate4()%>&status=<%=p.getP_share_count()%>&num=2"  style='color:#666666;'><%= p.getP_name() %></a></h3>
		 
        </div>
        
            <div class="price" style="float: right;">
                <% if(p.getP_share_count() ==2){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>입금완료</div>	 
            								 <%} %>
            								 
            								  <% if(p.getP_share_count() ==1){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>입금대기</div>	 
            								 <%} %>
     
      			 <% if(p.getP_share_count() ==3||p.getP_share_count()==8){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>공유완료</div>	 
            								 <%} %>
     
     		<% if(p.getP_share_count() ==4){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>거래완료</div>	 
            								 <%} %>
     
     	<% if(p.getP_share_count() ==5){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>수정요청</div>	 
            								 <%} %>
            								 
            								 <% if(p.getP_share_count() ==6){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>구매취소</div>	 
            								 <%} %>
            								 
            								  <% if(p.getP_share_count() ==7){ %>
               <div class="price_box m_top15 co2fa0ee bgnone" id="record_status" style='top:25px'>판매취소</div>	 
            								 <%} %>
   ...
   <!-- else 들어갈 곳 -->
         ...
         function UpdateRequest(data){
        	 ...
        	 location.href="<%=request.getContextPath() %>/updateDealer.do?record_code="+data;
         }
          
         function finishBuying(data){
        	 if(data){
        		 console.log("거래완료")
        	 }
      
      
        	 location.href="<%=request.getContextPath() %>/FinishBuying.do?record_code="+data;
         }
         </script>
